...
class Participantes : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val imageButtonFlechaRegresar=findViewById<ImageButton>(R.id.imageButton_Participantes_FlechaRegresar)
        val recyclerViewListaUsuarios=findViewById<RecyclerView>(R.id.recyclerView_Participantes_ListaUsuarios)
                ...
                p0.children.forEach{
                    ...
                    if (user!= null){
                        //if(user.uid != refUsuarioLoggeado) {
                            adapter.add(UserParticipantes(user))
                        //}
                    }
...
class UserParticipantes(val user: Registrarse.User?): Item<ViewHolder>(){
